...
function sideMenu(e){
	...
	var contentToShow = $(`.dadamanga-special-card-container > div[data-index="${dataIndex}"]`);
...
$(".focus-card-containers > .card").hover((e) => {
...
function nextCard(changeBrownTitle) {
    ...
    $(activeImg).removeClass(`animated wow fadeInLeft active ${imgLeftInAnimation} ${imgRightInAnimation}`);
    ...
    $(activeMonths).removeClass(`animated active ${monthsLeftInAnimation} ${monthsRightInAnimation}`);
    ...
    $(activeDescription).removeClass(`animated active ${descriptionLeftInAnimation} ${descriptionRightInAnimation}`);

    $(activeDescription).addClass(`animated ${descriptionRightOutAnimation} `);
    setTimeout(() => {
        $(activeDescription).addClass("d-none");
        $(activeDescription).removeClass(`animated ${descriptionRightOutAnimation} `);
        ...
        $(nextDescription).addClass(`animated ${descriptionRightInAnimation}  active`);
    }, delai);

    if (changeBrownTitle == true) {
        var activeTitle = $(".dadamanga-special-card-brown-title > .active");
        $(activeTitle).removeClass(`animated active ${descriptionLeftInAnimation} ${descriptionRightInAnimation}`);
...
function prevCard(changeBrownTitle) {
    ...
    setTimeout(() => {
        $(activeDescription).addClass("d-none");
        $(activeDescription).removeClass(`animated ${descriptionLeftOutAnimation} `);
        ...
        $(prevDescription).addClass(`animated ${descriptionLeftInAnimation}  active`);
    }, delai);
...
$(document).ready(() => {
    $(".overlay-marron-container").hover((e) => {
    ...
    if ($('#galleria').length) {
        ...
        Galleria.ready(function (options) {
            ...
            Galleria.get(0).$('thumb-nav-right').click(function () {
                ...
                var $flag = true;
                if (total_sec <= tracked_sec_index) { // add 1 to index bcoz index start from 0
                    Galleria.get(0).$('thumb-nav-right').hide();
                    $flag = false;
