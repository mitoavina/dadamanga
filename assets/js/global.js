...
function slideInit() {
    $(`.slide-btn-previous`).css({ 'display': 'flex' });
    $(`.slide-btn-next`).css({ 'display': 'flex' });
    // $(`#adventure-modal-title`).text(title[slidePosition - 1]);


    // $(`#adventure-modal-title`).removeClass("advenutre-modal-header-title");
    $(`.dot-content`).css({ 'display': 'block' });
    $(`.slide-body-header`).css({ 'display': 'flex' });
    $(`.slide-btn-next`).text("Next");
    $(`.slide-btn-change`).css({ 'display': 'none' });
    $(`.slide-btn-send`).css({ 'display': 'none' });
}
...
function slide(n) {
    ...
    switch (slidePosition) {
        case 0:
            $(`.slide-btn-previous`).css({ 'display': 'none' });
            break;
        case slides.length - 2:
            $(`.modal-footer .slide-btn-next`).text("Finish");
            break;
        case slides.length - 1:
            $(`.slide-btn-change`).css({ 'display': 'flex' });
            $(`.slide-btn-send`).css({ 'display': 'flex' });


            $(`.slide-btn-next`).css({ 'display': 'none' });
            $(`.dot-content`).css({ 'display': 'none' });
            $(`.slide-body-header`).css({ 'display': 'none' });
    ...
    slides.eq(slidePosition).css({ "display": "flex" });
    dot.eq(slidePosition).addClass("activeDot");
}

/**
 * For date picker need to import:
 * jquery-1.1.js
 * jquery-ui.js
 * jquery-ui.css
 */
