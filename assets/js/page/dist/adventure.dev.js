...
function init() {
  ...
  if ($.cookie("adventure") != undefined && $.cookie("adventure") != null && $.cookie("adventure") != "") {
    ...
    for (i in keys) {
      ...
      if (listAdventure[keys[i]].length != 0) numberOfAdventure(keys[i], listAdventure[keys[i]].length);
    }
        ...
        for (j in adventures[i]) {
          ...
          if (tmp == $(this).val()) {
            for (var k in listAdventure[keys[i]]) {
              if (adventures[i][j].id == listAdventure[keys[i]][k].id) return;
            }
            ...
            setCartNumber();
            if (show) $(".show-adventure-container .show-adventure-btn").fadeOut("normal", function () {
              $(this).remove();
            });
            return;
          }
      ...
      $(this).on("click", function () {
        ...
        for (i in keys) {
          for (j in listAdventure[keys[i]]) {
            ...
            if (tmp == $(this).val()) {
              $(".your-adventure-content > div[data-index=\"".concat($(this).val(), "\"]")).fadeOut("normal", function () {
                $(this).remove();
              });
...
function createCookie() {
  ...
  $.cookie("adventure", JSON.stringify(listAdventure), {
    expires: date
  });
}
...
var slideIndex = 1;
var title = ['THE ADVENTURE BEGINS..', 'THE ADVENTURE CONTINUES..', 'ALMOST DONE', 'YOUR ADVENTURE'];
slide(slideIndex);
...
function slide(n) {
  ...
  $(".adventure-btn-previous").css({
    'display': 'flex'
  });
  $(".adventure-btn-next").css({
    'display': 'flex'
  });
  ...
  $(".adventure-modal-body-header").css({
    'display': 'flex'
  });
  $(".adventure-btn-next").text("Next");
  $(".adventure-btn-change").css({
    'display': 'none'
  });
  $(".adventure-btn-send").css({
    'display': 'none'
  });
  if (slideIndex == 1) $(".adventure-btn-previous").css({
    'display': 'none'
  });
  ...
  if (slideIndex == slides.length - 1) $(".modal-footer .adventure-btn-next").text("Finish");
